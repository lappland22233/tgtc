...
### 认证
- 邮箱注册与登录，邮件验证码（可配置开关）
- JWT 身份认证（HttpOnly Cookie），三级角色权限

### 文件管理
- 拖拽上传 / 弹窗批量上传
- 文件列表搜索、分页、类型筛选
- 设置公开/私有、访问次数限制、分享有效期
- 批量勾选图片一键生成 Markdown 链接
- 后端代理下载（不暴露 Telegram 原始 URL）

### 分享访问
- **公开无约束**：直接返回文件内容，CDN 友好
- **加密文件**：访问时弹出密码验证页面，输入 `?ps=` 参数访问
- **受限文件**（次数限制/时效限制）：自动 302 跳转一次性访问链接
...
│   ├── telegram/            # Telegram Bot API 上传下载
│   ├── mailer/              # SMTP 邮件
│   ├── config/              # 动态配置缓存
│   ├── tasks/               # 定时清理过期封禁
│   ├── common/
...
| GET | `/files/public/:id?access=` | 一次性访问链接（30秒有效期） |
