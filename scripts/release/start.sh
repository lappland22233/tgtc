...
install_services() {
  ...
  # 单元以 root 运行（更新器需要 systemctl/写 releases），入口固定为本发行包的 updater.sh，
  # 任务 ID 是 systemd 转义的 %i（uuid）。后端通过 `systemctl start --no-block` 触发。
  cat > "/etc/systemd/system/tgtc-update@.service" <<EOF
[Unit]
...
[Service]
Type=oneshot
ExecStart=$current_q/scripts/release/updater.sh %i
TimeoutStartSec=1800
StandardOutput=journal
StandardError=journal
EOF

  # polkit 限定：服务用户仅可 start 名称匹配 tgtc-update@<uuid>.service 的单元，
  # 不能 stop/restart/管理其他单元；比 sudoers 更贴合 NoNewPrivileges 后端。
  install -d -m 0755 /etc/polkit-1/rules.d
  cat > "/etc/polkit-1/rules.d/49-tgtc-update.rules" <<POLKIT
// Generated by TGTC start.sh. Grants the TGTC service user the right to
// start its own update oneshot units (tgtc-update@<task-uuid>.service) only.
polkit.addRule(function(action, subject) {
  if (action.id === 'org.freedesktop.systemd1.manage-units' &&
      subject.user === '$service_user' &&
      /^tgtc-update@[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\\.service\$/.test(action.lookup('unit') || '') &&
...
EOF
  ...
  unit_files+=("/etc/systemd/system/tgtc-update@.service")
...
# P1-02：configure 会向 $RUNTIME_DIR/backend/.env 与 $RUNTIME_DIR/telegram-bot-api/.env
# 写入配置，而目录原先要到 prepare_permissions 才创建——全新部署 100% 失败。
# 在进入首次配置前先创建运行时目录（所有权由 prepare_permissions 统一规范化）。
ensure_runtime_dirs() {
  install -d -m 0750 "$RUNTIME_DIR/backend" "$RUNTIME_DIR/telegram-bot-api"
}
