...
const uuid_subscriber_1 = require("./uuid.subscriber");
/**
...
function createDatabaseOptions(env = process.env) {
    const type = (env.DB_TYPE || 'postgres').toLowerCase();
    if (type !== 'postgres' && type !== 'sqlite') {
        throw new Error(`DB_TYPE 必须是 postgres 或 sqlite，当前值: ${type}`);
    }
    const migrations = [(0, path_1.join)(__dirname, '..', 'migrations', '*{.ts,.js}')];
    if (type === 'sqlite') {
        // PG 历史迁移包含 CREATE EXTENSION、ILIKE 等 SQLite 不支持的方言；SQLite 使用独立基线。
        const sqliteMigrations = [(0, path_1.join)(__dirname, '..', 'migrations', '0000000000000-SqliteEntitySchema*{.ts,.js}')];
        return {
            type: 'sqlite',
            database: env.DB_DATABASE || 'data/tgtc.sqlite',
            entities: [...entities_1.databaseEntities],
            subscribers: [uuid_subscriber_1.UuidSubscriber],
