...
exports.SqliteEntitySchema1700000000000 = void 0;
const typeorm_1 = require("typeorm");
function sqliteColumnType(column) {
    ...
    if (column.type === Date)
        return 'datetime';
    if (column.type === 'uuid' || column.type === 'enum' || column.type === 'jsonb' || column.type === 'simple-json')
        return 'varchar';
...
function sqliteColumnDefault(column) {
    if (typeof column.default !== 'string' && typeof column.default !== 'number' && typeof column.default !== 'boolean')
        return undefined;
    if (typeof column.default === 'boolean')
        return column.default ? '1' : '0';
    if (typeof column.default === 'number')
        return String(column.default);
    if (/^\(?\s*(CURRENT_TIMESTAMP|CURRENT_DATE|CURRENT_TIME)\s*\)?$/i.test(column.default))
        return column.default;
    if (/^'.*'$/.test(column.default))
        return column.default;
    return `'${column.default.replace(/'/g, "''")}'`;
}
/** SQLite 的可运行基线迁移：从实体元数据生成 DDL，但不调用 synchronize。 */
class SqliteEntitySchema1700000000000 {
    constructor() {
        this.name = 'SqliteEntitySchema1700000000000';
    }
    async up(queryRunner) {
        ...
        for (const metadata of metadatas) {
            ...
            await queryRunner.createTable(new typeorm_1.Table({
                ...
                columns: metadata.columns.map((column) => new typeorm_1.TableColumn({
        ...
        for (const metadata of metadatas) {
            for (const unique of metadata.uniques) {
                ...
                if (!table || table.uniques.some((existing) => existing.name === unique.name))
                    continue;
            ...
            for (const relation of metadata.relations) {
                if (!relation.isManyToOne || !relation.joinColumns.length)
                    continue;
                const join = relation.joinColumns[0];
                const target = relation.inverseEntityMetadata;
                const table = await queryRunner.getTable(metadata.tableName);
                if (!table || table.foreignKeys.some((key) => key.columnNames.includes(join.databaseName)))
                    continue;
                await queryRunner.createForeignKey(metadata.tableName, new typeorm_1.TableForeignKey({
...
exports.SqliteEntitySchema1700000000000 = SqliteEntitySchema1700000000000;
