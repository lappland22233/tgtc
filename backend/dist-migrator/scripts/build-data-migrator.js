...
const _1800000000000_SqliteSchemaAlignment_1 = require("../src/migrations/1800000000000-SqliteSchemaAlignment");
const data_migrator_utils_1 = require("./data-migrator-utils");
...
function expectedAffinity(type) {
...
function convertValue(value, sourceColumn, targetColumn) {
    ...
    if (isSimpleJson(column)) {
        if (typeof value === 'string') {
            try {
                ...
                throw new Error(`simple-json 值不是合法 JSON: ${column.entityMetadata.tableName}.${column.databaseName}`);
            }
        }
    ...
    if (isDateColumn(column)) {
        ...
        if (Number.isNaN(date.getTime()))
            throw new Error(`datetime 值无法转换: ${column.entityMetadata.tableName}.${column.databaseName}`);
        return date.toISOString();
    }
    if (column.type === 'bigint') {
        const text = String(value);
        if (!/^-?\d+$/.test(text))
            throw new Error(`bigint 值不是整数: ${column.entityMetadata.tableName}.${column.databaseName}`);
...
async function buildPlans(source, sourceSnapshot, target) {
    ...
    for (const definition of data_migrator_utils_1.NON_ENTITY_TABLE_DEFINITIONS) {
        if (!(await postgresTableExists(sourceSnapshot, definition.table)))
            continue;
        plans.push({
...
async function migrateTable(source, target, plan) {
    const { table, columns, primaryColumns } = plan;
    const sourceCount = await count(source, table);
    if (!primaryColumns.length) {
        if (sourceCount > 0)
            throw new Error(`表 ${table} 无主键且包含 ${sourceCount} 行；无法提供稳定 keyset 快照，拒绝隐式 OFFSET 迁移`);
        return { table, mode: 'empty-no-key', primaryColumns: [], maxPrimaryKey: null, sourceCount, copied: 0 };
    }
    const maxRows = sourceCount === 0 ? [] : await source.query(`SELECT ${primaryColumns.map(quote).join(', ')} FROM ${quote(table)} ORDER BY ${primaryColumns.map((column) => `${quote(column)} DESC`).join(', ')} LIMIT 1`);
    ...
    while (maxPrimaryKey) {
        ...
        const rows = await source.query(`SELECT ${columns.map(quote).join(', ')} FROM ${quote(table)} WHERE ${conditions.join(' AND ')} ORDER BY ${primaryColumns.map(quote).join(', ')} LIMIT $${parameters.length}`, parameters);
        if (!rows.length)
            break;
...
function metadataExpectation(metadata) {
    ...
    for (const unique of metadata.uniques) {
        ...
        if (columns.length)
            indexes.push({ columns, unique: true });
    }
    for (const column of metadata.columns.filter((item) => item.isUnique)) {
...
function nonEntityExpectations() {
    return data_migrator_utils_1.NON_ENTITY_TABLE_DEFINITIONS.map((definition) => ({
        table: definition.table,
...
async function ensureNonEntityTables(target, plans) {
    const plannedTables = new Set(plans.map((plan) => plan.table));
    for (const definition of data_migrator_utils_1.NON_ENTITY_TABLE_DEFINITIONS) {
        if (!plannedTables.has(definition.table))
            continue;
        await target.query(definition.createSql);
        for (const sql of definition.indexSql)
            await target.query(sql);
    }
}
async function readIndex(target, table, index) {
    const columns = await target.query(`PRAGMA index_info(${quote(index.name)})`);
...
async function validateSchema(target, expectations) {
    const tables = {};
    for (const expected of expectations) {
        if (!(await tableExists(target, expected.table)))
            throw new Error(`schema/table 校验失败: 缺少表 ${expected.table}`);
        const columns = await target.query(`PRAGMA table_info(${quote(expected.table)})`);
        ...
        for (const expectedColumn of expected.columns) {
            ...
            if (!actual)
                throw new Error(`schema/列校验失败: ${expected.table}.${expectedColumn.name} 缺失`);
        ...
        for (const expectedIndex of expected.indexes) {
            const found = actualIndexes.some((actual) => (0, data_migrator_utils_1.sqliteIndexMatches)(actual, expectedIndex));
            if (!found) {
...
async function validateData(target, snapshots) {
    const tables = {};
...
async function validateIntegrity(target) {
    ...
    if (foreignKeys.length)
        throw new Error(`integrity/外键校验失败: ${JSON.stringify(foreignKeys.slice(0, 20))}`);
    const integrityRows = await target.query('PRAGMA integrity_check');
    const messages = integrityRows.flatMap((row) => Object.values(row).map(String));
...
async function main() {
    ...
    if (targetOptions.type !== 'sqlite')
        throw new Error('迁移目标配置必须为 SQLite');
    const target = new typeorm_1.DataSource({
        ...targetOptions,
        database: tempPath,
        migrationsRun: true,
        migrations: [_0000000000000_SqliteEntitySchema_1.SqliteEntitySchema1700000000000, _1800000000000_SqliteSchemaAlignment_1.SqliteSchemaAlignment1800000000000],
    });
    const report = {
        summary: {
            status: 'running',
            startedAt: new Date().toISOString(),
            source: String(sourceOptions.database || ''),
            target: targetPath,
    ...
    try {
        ...
        await (0, data_migrator_utils_1.beginPostgresReadOnlySnapshot)(sourceSnapshot);
        ...
        for (const plan of plans)
            report.migration.tables.push(await migrateTable(sourceSnapshot, target, plan));
        ...
        (0, data_migrator_utils_1.publishMigrationArtifacts)(tempPath, targetPath, backupPath, `${tempPath}.report.json`, reportPath, reportBackupPath);
        ...
        if (sourceSnapshot) {
            if (sourceTransactionStarted || sourceSnapshot.isTransactionActive) {
                try {
                    await sourceSnapshot.rollbackTransaction();
                }
