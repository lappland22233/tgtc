...
export interface UserRelayRequest {
   ...
   * 它同时决定 BT 选号种子与 MTProto `random_id`，从而保证「重试不产生重复消息」。
   * 缺省时退化为 `sourceChatId:sourceMessageId`（同一源消息天然幂等）。
   */
  idempotencyKey?: string;
}

/** 中继失败分类（供调用方记录与排障，不影响降级决策） */
export type UserRelayFailureReason =
  | 'user_relay_not_configured'
  | 'user_relay_unavailable'
  | 'user_relay_source_missing'
  | 'user_relay_target_missing'
  | 'user_relay_no_account'
  | 'user_relay_execution_failed';
 ...
 *   `TelegramMirrorRelaySourceService`。
 *
...
export class UserRelayService {
  ...
  constructor(
    ...
    /**
     * 镜像规则仓库：用于把「下载期懒扩散」的中继目标锚定到实际备份群。
     *
     * 用 `@Optional()`：大量单测直接 new 本服务；缺失时回退 `TELEGRAM_ARCHIVE_CHAT_ID`。
     */
    ...
    if (this.enabled && !this.userClient.isAvailable()) {
      this.logger.error(
        ...
        + '用户账号中继将被跳过，副本扩散自动回退「逐账号二次上传」（策略 A）。',
      );
    }
  }
   ...
   * 首次中继被静默跳过。真实账号可用性由 `relay()` 判定并给出 `user_relay_no_account`
   ...
   * 解析中继目标群（副本可见群）。
   *
   * 优先级：**启用中的镜像规则备份群** > `TELEGRAM_ARCHIVE_CHAT_ID`。
   *
   * 为什么优先规则：镜像规则的 `targetChatId` 才是「全部 Bot 可见」这一语义的权威来源
   * （规则启用前必须通过真实权限探测）；只配规则、不配归档群时若不做这一步，
   * 下载期懒扩散的中继会因 `user_relay_target_missing` 静默回退策略 A。
   */
  async resolveTargetChatId(): Promise<string> {
    if (this.rules) {
      try {
        ...
        if (this.cachedRuleTarget) return this.cachedRuleTarget;
      } catch (error) {
        this.logger.warn(
          `镜像备份群读取失败（回退 TELEGRAM_ARCHIVE_CHAT_ID）：`
   ...
   *
   * 失败一律返回 `{ok:false, reason}` 而**不抛出**：调用方（副本扩散）会据此自动回退到
   * 策略 A（逐账号二次上传），中继不可用不得影响文件可用性。
   */
  async relay(request: UserRelayRequest): Promise<UserRelayResult> {
    // 「未开启开关」不算失败（是明确的产品状态），不计数；其余失败**全部**计入 userRelaysFailed，
    // 否则「已配置但完全不可用」在告警面板上完全无声（启动预检只告警不阻断）。
    if (!this.enabled) {
      return { ok: false, reason: 'user_relay_not_configured', detail: 'TELEGRAM_USER_RELAY_ENABLED 未开启' };
    }
    if (!this.userClient.isAvailable()) {
      this.pool.bumpCounter('userRelaysFailed');
      return {
        ok: false,
        reason: 'user_relay_unavailable',
        detail: this.userClient.unavailableReason() ?? 'MTProto 客户端不可用',
      };
    }
    ...
    if (!sourceChatId || !sourceMessageId) {
      this.pool.bumpCounter('userRelaysFailed');
      return { ok: false, reason: 'user_relay_source_missing', detail: '缺少源消息定位（chat_id + message_id）' };
    }
    if (!targetChatId) {
      this.pool.bumpCounter('userRelaysFailed');
      return {
        ok: false,
        reason: 'user_relay_target_missing',
        detail: '未配置副本可见群（TELEGRAM_ARCHIVE_CHAT_ID 为空且调用方未指定目标群）',
      };
    }
    ...
    if (accounts.length === 0) {
      ...
      return {
        ok: false,
        reason: 'user_relay_no_account',
        detail: '没有可用（已授权且启用）的 Telegram 用户账号',
      };
    }
    ...
    try {
      ...
      this.pool.bumpCounter('userRelaysOk');
      this.logger.log(
        `用户账号中继成功：账号 ${chosen.id} / 源 ${this.mask(sourceChatId)}→目标 ${this.mask(targetChatId)}`
        + ` / 新消息 ${copied.targetMessageId}`,
      );
      return { ok: true, messageId: copied.targetMessageId, accountId: chosen.id };
    } catch (error) {
      this.pool.bumpCounter('userRelaysFailed');
      const message = error instanceof Error ? error.message : String(error);
      // 返回值会进入任务摘要/审计等展示面，因此只回传**已脱敏**的分类摘要：
      ...
      const kind = isClientError ? error.kind : 'other';
      const detail = isClientError
        ? `${kind}: ${message.slice(0, 200)}`
        : 'other: 用户账号中继执行失败（详见后端日志）';
      // 凭据失效必须让管理员可见：否则账号一直显示 active，中继却持续失败，运维无从下手
      if (kind === 'auth') {
        await this.directory.markDegraded(chosen.id, 'user_session_invalid', message);
      }
      this.logger.warn(`用户账号中继失败（账号 ${chosen.id} / ${kind}）：${message.slice(0, 200)}`);
      return { ok: false, reason: 'user_relay_execution_failed', detail };
    }
  }
