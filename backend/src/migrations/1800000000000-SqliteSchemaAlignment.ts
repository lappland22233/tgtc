 ...
 */
export class SqliteSchemaAlignment1800000000000 implements MigrationInterface {
  name = 'SqliteSchemaAlignment1800000000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const metadata of queryRunner.connection.entityMetadatas) {
      ...
      for (const column of metadata.columns) {
        ...
        if (!canAddToPopulatedTable) {
          const countRows = await queryRunner.query(`SELECT COUNT(*) AS "count" FROM "${metadata.tableName.replace(/"/g, '""')}"`) as Array<{ count: number | string }>;
          if (Number(countRows[0]?.count || 0) > 0) {
            throw new Error(
              `SQLite 增量迁移无法安全新增非空无默认列 ${metadata.tableName}.${column.databaseName}：表中已有数据；请编写专用迁移先回填再收紧约束`,
            );
          }
        }
        await queryRunner.addColumn(metadata.tableName, new TableColumn({
      ...
      const uniqueDefinitions = [
        ...
        ...metadata.columns
          .filter((column) => (column as typeof column & { isUnique?: boolean }).isUnique)
      ...
      for (const unique of uniqueDefinitions) {
        if (!unique.columns.length) continue;
        const current = await queryRunner.getTable(metadata.tableName);
        if (!current) continue;
        const sameColumns = (columns: readonly string[]) => columns.join('\0') === unique.columns.join('\0');
        ...
        const duplicates = await queryRunner.query(
          `SELECT ${quotedColumns.join(', ')}, COUNT(*) AS "duplicateCount"
        ...
        if (duplicates.length) {
          throw new Error(
            `SQLite 增量迁移无法补齐唯一约束 ${metadata.tableName}(${unique.columns.join(', ')}): `
      ...
      for (const index of metadata.indices) {
        ...
        if (refreshed.indices.some((existing) => existing.name === index.name)) continue;
    ...
    if (await queryRunner.hasTable('folders')) {
      ...
      if (duplicates.length) {
        throw new Error(
          'SQLite 增量迁移无法补齐唯一索引 folders(ownerId, COALESCE(parentId, \'\'), name) WHERE isDeleted=false: '
    ...
    if (await queryRunner.hasTable('folder_closure')) {
      ...
      if (table && !table.indices.some((index) => index.name === 'IDX_closure_descendant')) {
        await queryRunner.createIndex('folder_closure', new TableIndex({
...
function sqliteColumnType(type: unknown): string {
  if (type === String) return 'varchar';
  if (type === Number) return 'integer';
  if (type === Boolean) return 'boolean';
  if (type === Date || type === 'timestamp' || type === 'timestamptz' || type === 'datetime') return 'datetime';
  if (type === 'uuid' || type === 'enum' || type === 'jsonb' || type === 'simple-json') return 'varchar';
  if (type === 'bigint') return 'bigint';
  return typeof type === 'string' ? type : 'varchar';
}

function sqliteColumnDefault(value: unknown): string | number | boolean | undefined {
  if (typeof value === 'boolean' || typeof value === 'number') return value;
  if (typeof value !== 'string') return undefined;
  if (/^\(?\s*(CURRENT_TIMESTAMP|CURRENT_DATE|CURRENT_TIME)\s*\)?$/i.test(value)) return value;
