...
/**
 * 先校验后端可访问性，确认 200 后通过 <a> 点击触发浏览器原生下载。
...
async function downloadFile(file: FileSummary) {
  ...
  try {
    ...
    // 先验证（仅读头，不读 body）
    const check = await fetch(url, { method: 'HEAD' }).catch(() => ({ ok: false, status: 0 } as Response));

    if (!check.ok && check.status !== 0 && check.status !== 405 && check.status !== 501) {
      throw new Error(`下载失败（${check.status}）`);
    }
